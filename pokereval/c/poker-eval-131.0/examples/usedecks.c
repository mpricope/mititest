...
int evalOmahaI(uint32  holeCards[],uint32 boardCards[],uint32 * o_hival,uint32 * o_lowval) {
  ...
  do {
	//printf("Int %d\n",holeCards[nhole]);
     if (!StdDeck_CardMask_CARD_IS_SET(hole, holeCards[nhole])) {
      StdDeck_CardMask_SET(hole, holeCards[nhole]);
      ++nhole;
    };
  } while (nhole < 4);
  ...
  do {
		//printf("Int %d\n",boardCards[nboard]);
	  if (!StdDeck_CardMask_CARD_IS_SET(board, boardCards[nboard])) {
      StdDeck_CardMask_SET(board, boardCards[nboard]);
      ++nboard;
    };
  } while (nboard < 5);
